...
.sf-menu a:focus, .sf-menu a:hover, .sf-menu a:active {background:'.$VAN['navi_link_bghover'].';}'.PHP_EOL; 
	   ...
	   if(isset($VAN['section_heading_font']) && $VAN['section_heading_font'] <> ''){
		$custom_css.='.page-area hgroup.title h1,.page-area hgroup.title h1 strong,.entry h1,.entry h2,.entry h3,.entry h4,.entry h5,.entry h6,.post h2{font-family:"'.str_replace('+', ' ', $section_heading_font[0]).'",Arial;}'.PHP_EOL;
	   }
